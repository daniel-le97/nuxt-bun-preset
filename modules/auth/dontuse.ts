...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup(userOptions, nuxt) {
    ...
    //   nitroConfig.alias['#auth'] = resolve('./runtime/lib/server/server')
    // })
    // addServerImports([
    //   { name: 'NuxtAuthHandler', from: resolve('./runtime/lib/server/server') },
    //   { name: 'getServerSession', from: resolve('./runtime/lib/server/server') },
    // ])
    addServerImportsDir(resolve('./runtime/lib/server'))
    ...
    addTypeTemplate({
      ...
      getContents: () => [
        ...
        `  const getServerSession: typeof import('${resolve('./runtime/lib/server/server')}').getServerSession`,
        `  const NuxtAuthHandler: typeof import('${resolve('./runtime/lib/server/server')}').NuxtAuthHandler`,
        `  const getServerToken: typeof import('${resolve('./runtime/lib/server/server')}').getServerToken`,
        '}',
      ].join('\n'),
    })
